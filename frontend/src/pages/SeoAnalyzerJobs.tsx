...
export default function SeoAnalyzerJobs() {
  ...
  if (!hasPermission('agents:trigger:seo-analyzer') && !hasPermission('*')) {
    return <div style={{ padding: 40, textAlign: 'center', color: '#697a82' }}>Access denied.</div>;
  }

  const { data, isLoading } = useQuery({
    queryKey: ['jobs', 'seo-analyzer', statusFilter, page],
    queryFn: () => api.getJobs({ agent: 'seo-analyzer', status: statusFilter || undefined, limit: PAGE, offset: page * PAGE }),
  ...
  return (
    <>
      <div className="crumb-row">
        <div className="breadcrumb"><span>MI OS</span><span>/</span><strong>SEO Analyzer</strong></div>
      ...
      <div className="page-titlebar">
        <div>
          <div className="title-kicker">Agent · SEO</div>
          <h1 className="page-title">SEO Analyzer Jobs</h1>
          <p className="page-sub">All runs of the SEO Analyzer agent. Click a row to view output.</p>
        </div>
      ...
      <div className="sn-card" style={{ padding: 0 }}>
        {isLoading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', color: '#697a82', fontSize: 13 }}>
            ...
            No SEO Analyzer jobs{statusFilter ? ` with status "${statusFilter}"` : ''}.
          </div>
        ) : (
